...
public class PetLikeAction extends ActionSupport implements ServletRequestAware {
	...
	public String execute() {
		...
		int number2 = Integer.parseInt(numberStr); // 將String轉成Integer(此number為正在感興趣的對象編號)
		...
		checkSelf=petService.selectId(petBean.get(number2).getPET_OWN_ID().toString(),selectKing.get(0).getPET_OWN_ID().toString(), "like");
		session.removeAttribute("match");
		
		if(!checkSelf.isEmpty()){
			session.setAttribute("match", petBean.get(number2).getPET_NAME());
		}
		...
		System.out.println("like="+session.getAttribute("PetNumber"));
		return "success";
